// Author: envy
// Problem: Labyrinth from CSES - CSES Problem Set
// URL: https://cses.fi/problemset/task/1193/
// Limits: 1000 ms, 512 MB
// Attempted: 2021-09-10 08:38:05 EST
...
void doStuff(){
	...
	for(int i = 0; i < n; i++){
		for(int j = 0; j < m; j++){
			rc(grid[i][j]);
			if(grid[i][j] == 'A')apos = mp(i,j);
			if(grid[i][j] == 'B')bpos = mp(i,j);
		}
	}
	...
	pq.push({0,apos.F,apos.S});
	dist[apos.F][apos.S] = 0;
	// [dist][posx][posy]
	while(!pq.empty()){
		array<int,3> now = pq.top();
		pq.pop();
		...
		for(int i = 0; i < 4; i++){
			...
			if(!visited[newr][newc]){
				...
				if(newdist < dist[newr][newc]){
					prev[newr][newc] = mp(now[1],now[2]);
					dist[newr][newc] = newdist;
					pq.push({newdist,newr,newc});
				}
			}
		}
		if(now[1] == bpos.F && now[2] == bpos.S){
			break;
		}
		
	}
	if(dist[bpos.F][bpos.S] == 1e9){
		print("NO");
		return;
	}
	...
	int posx = bpos.F;
	int posy = bpos.S;
	pair<int,int> previous = prev[bpos.F][bpos.S];
	while(dist[bpos.F][bpos.S]--){
		debug(previous);
		if(previous.F != posx){
			if(previous.F > posx){
				path.pb('U');
		...
		else if(previous.S != posy){
			if(previous.S > posy) path.pb('L');
			else path.pb('R');
		}
		posx = previous.F;
		posy = previous.S;
		previous = prev[previous.F][previous.S];
	}
	reverse(all(path));
	print("YES");
	print(path.size());
