// Author: envy
// Problem: Nested Ranges Check from CSES - CSES Problem Set
// URL: https://cses.fi/problemset/task/2168
// Limits: 1000 ms, 512 MB
// Attempted: 2021-09-12 13:42:09 EST
...
void doStuff(){
	...
	for(int i = n-2; i >= 0; i--){
		if(nums[i].S < miny){
			...
			contains[index[nums[i]]] = true;
		}
	}
	
	int maxy = nums[0].S;
	int indexnow = index[nums[0]];
	for(int i = 1; i < n; i++){
		if(nums[i].S > maxy){
			maxy = nums[i].S;
			indexnow = index[nums[i]];
		}
		else{
			iscontained[index[nums[i]]] = true;
		}
	}
