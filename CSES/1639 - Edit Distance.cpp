// Author: envifly
// Problem: Edit Distance from CSES - CSES Problem Set
// URL: https://cses.fi/problemset/task/1639
// Limits: 1000 ms, 512 MB
// Attempted: 2021-08-30 18:05:29 EST
...
void doStuff(){
	...
	for(int i = 1; i <= n; i++){
		for(int j = 1; j <= m; j++){
			dp[i][j] = min({dp[i-1][j]+1, dp[i-1][j-1] + (a[i-1] != b[j-1]), dp[i][j-1] + 1});
		}
	}
	print(dp[n][m]);
